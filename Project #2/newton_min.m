function [x_k_arr, y_k_arr, num_of_iter, x_min] = newton_min(f, x0, y0, epsilon)
    % Newton method with gamma such that it minimizes f(x + g*d)
    %
    % Inputs: 
    % f: Handle function f
    ...
    fprintf(['Minimum of f with starting points at (%.0f, %.0f) ' ...
        'using the Newton method: (%0.5f, %0.5f)\n'], x0, y0, x_min(1), x_min(2));
end
