function [x_k_arr, y_k_arr, num_of_iter, x_min] = levenberg_marquardt_armijo(f, x0, y0, epsilon)
    % Levenberg-Marquardt method with gamma chosen by the Armijo rule
    %
    % Inputs: 
    % f: Handle function f
    ...
    while (norm(f_grad) >= e)
        ...
        while f(x + g*d) > f(x) + a*g*d*f_grad %
            g = g/2;                           % Criterion 4 of 5.2.6
        end                                    %
        % Step 5
        x = x + g*d;
        f_grad = grad_approx(f,x);
    ...
    fprintf(['Minimum of f with starting points at (%.0f, %.0f) ' ...
        'using the Levenberg - Marquardt method and the Armijo rule: (%0.5f, %0.5f)\n'], x0, y0, x_min(1), x_min(2));
end